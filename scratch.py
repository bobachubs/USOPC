...
def advance_team_AA(team_AA_scores, n):
    ...
    for country, athletes in team_AA_scores.items():
        country_total[country] = dict()
        for athlete in athletes:
            athlete_totals = {'Score': 0,
                            'D_Score': 0,
                            'E_Score': 0,
                            'Country': country}
            for _, app_scores in athletes[athlete].items():
                athlete_totals['Score'] += app_scores['Score']
                athlete_totals['D_Score'] += app_scores['D_Score']
                athlete_totals['E_Score'] += app_scores['E_Score']
            country_total[country][athlete] = athlete_totals
        print(country_total["USA"])
        sorted_athletes = sorted(country_total[country].items(), key=lambda item: item[1]['Score'], reverse=True)[:3]
        country_sum = {'Score': 0,
                        'D_Score': 0,
                        'E_Score': 0,
                    'Country': country}       
        print(sorted_athletes)
    for athlete, app_scores in sorted_athletes:
        country_sum['Score'] += app_scores['Score']
        country_sum['D_Score'] += app_scores['D_Score']
        country_sum['E_Score'] += app_scores['E_Score']
    ...
    teams_advance = [country for country,_ in country_scores_sum]
...
# future 
# consider weighting more recent scores?
# tie breaking
# sample by rounds
# mix model
